 ...
 * Usage : npx tsx src/db/seed-neon.ts
 * 
 * Prérequis : DATABASE_URL doit être définie dans l'environnement.
 * Exemple : DATABASE_URL="postgresql://..." npx tsx src/db/seed-neon.ts
 */
...
if (!process.env.DATABASE_URL) {
  ...
  console.error('   Usage : set DATABASE_URL=postgresql://... && npm run seed:neon');
  process.exit(1);
}

async function seedNeon() {
  console.log('🌱 Seed Neon PostgreSQL...\n');
  ...
  const tables = ['historical_standings', 'transfers', 'predictions', 'standings', 'matches', 'players', 'teams', 'leagues'];
  ...
  // ── Insérer les données ──
  // Note: on utilise dbRun avec des INSERT au lieu de dbExec pour les queries paramétrées
  // Mais pour simplifier, on utilise dbExec avec des strings SQL complètes

  console.log('📥 Insertion des ligues...');
  await dbExec(`
  ...
  console.log('📥 Insertion des classements...');
  await dbExec(`
  ...
  console.log('\n✅ Seed Neon terminé avec succès !');
}

seedNeon().catch(err => {
  console.error('❌ Erreur seed Neon:', err);
  process.exit(1);
});
