...
async function runNeonSeed() {
  ...
  // Fonction utilitaire : exécute une requête SQL sans paramètres
  const exec = async (query: string) => { await sql.unsafe(query); };

  // ── Créer les tables ──
  await exec(`CREATE TABLE IF NOT EXISTS leagues (id TEXT PRIMARY KEY, name TEXT NOT NULL, slug TEXT NOT NULL UNIQUE, country TEXT NOT NULL, flag TEXT NOT NULL, season TEXT NOT NULL)`);
  await exec(`CREATE TABLE IF NOT EXISTS teams (id TEXT PRIMARY KEY, name TEXT NOT NULL, slug TEXT NOT NULL, short_name TEXT, country TEXT, flag TEXT, league_id TEXT NOT NULL, stadium TEXT, capacity INTEGER, coach TEXT, budget REAL, founded_year INTEGER, market_value TEXT, color TEXT)`);
  await exec(`CREATE TABLE IF NOT EXISTS players (id TEXT PRIMARY KEY, name TEXT NOT NULL, position TEXT NOT NULL, number INTEGER, nationality TEXT, flag TEXT, team_id TEXT NOT NULL, market_value TEXT, goals INTEGER DEFAULT 0, assists INTEGER DEFAULT 0, appearances INTEGER DEFAULT 0, rating REAL)`);
  await exec(`CREATE TABLE IF NOT EXISTS matches (id TEXT PRIMARY KEY, league_id TEXT NOT NULL, home_team_id TEXT NOT NULL, away_team_id TEXT NOT NULL, date TEXT NOT NULL, time TEXT, round TEXT, venue TEXT, status TEXT DEFAULT 'upcoming', home_score INTEGER, away_score INTEGER, home_formation TEXT, away_formation TEXT)`);
  await exec(`CREATE TABLE IF NOT EXISTS standings (id TEXT PRIMARY KEY, league_id TEXT NOT NULL, team_id TEXT NOT NULL, position INTEGER NOT NULL, played INTEGER DEFAULT 0, won INTEGER DEFAULT 0, drawn INTEGER DEFAULT 0, lost INTEGER DEFAULT 0, goals_for INTEGER DEFAULT 0, goals_against INTEGER DEFAULT 0, goal_difference INTEGER DEFAULT 0, points INTEGER DEFAULT 0, form TEXT)`);
  await exec(`CREATE TABLE IF NOT EXISTS predictions (id TEXT PRIMARY KEY, match_id TEXT NOT NULL, prediction_type TEXT NOT NULL, confidence REAL NOT NULL, odds REAL, reasoning TEXT, btts TEXT, btts_confidence REAL, over_under TEXT, over_under_confidence REAL, created_at TEXT)`);
  await exec(`CREATE TABLE IF NOT EXISTS transfers (id TEXT PRIMARY KEY, player_id TEXT NOT NULL, from_team_id TEXT, to_team_id TEXT NOT NULL, date TEXT NOT NULL, fee TEXT, type TEXT NOT NULL, season TEXT NOT NULL)`);
  await exec(`CREATE TABLE IF NOT EXISTS historical_standings (id TEXT PRIMARY KEY, team_id TEXT NOT NULL, league_id TEXT NOT NULL, season TEXT NOT NULL, position INTEGER NOT NULL, points INTEGER DEFAULT 0)`);

  // ── Vider les tables ──
  for (const table of ['players', 'standings', 'matches', 'predictions', 'transfers', 'historical_standings', 'teams', 'leagues']) {
    try { await exec(`DROP TABLE IF EXISTS ${table} CASCADE`); } catch {}
  }
  // Re-créer les tables
  await exec(`CREATE TABLE IF NOT EXISTS leagues (id TEXT PRIMARY KEY, name TEXT NOT NULL, slug TEXT NOT NULL UNIQUE, country TEXT NOT NULL, flag TEXT NOT NULL, season TEXT NOT NULL)`);
  await exec(`CREATE TABLE IF NOT EXISTS teams (id TEXT PRIMARY KEY, name TEXT NOT NULL, slug TEXT NOT NULL, short_name TEXT, country TEXT, flag TEXT, league_id TEXT NOT NULL, stadium TEXT, capacity INTEGER, coach TEXT, budget REAL, founded_year INTEGER, market_value TEXT, color TEXT)`);
  ...
  // ── Insérer les ligues ──
  await exec(`INSERT INTO leagues (id, name, slug, country, flag, season) VALUES
  ...
  // ── Insérer les équipes ──
  await exec(`INSERT INTO teams (id, name, slug, short_name, country, flag, league_id, stadium, capacity, coach, budget, founded_year, market_value, color) VALUES
  ...
  await exec(`INSERT INTO standings (id, league_id, team_id, position, played, won, drawn, lost, goals_for, goals_against, goal_difference, points, form) VALUES
